...
const containerVariants = {
  ...
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.1 // 子元素間隔 0.1秒 出現
    }
  }
};
...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    ...
    showNotification(`訂單已送出！使用 ${paymentMethod} 付款`);
  ...
  if (cartItems.length === 0) {
    return (
      <motion.div 
        ...
        className="container mx-auto px-4 py-20 text-center min-h-[60vh] flex flex-col items-center justify-center"
      >
        <h2 className="text-4xl font-bold font-league mb-4 text-gray-800">Your Cart is Empty</h2>
        <p className="text-gray-500 mb-8">看起來你還沒選購任何美食，快去看看菜單吧！</p>
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-4xl md:text-5xl font-bold font-league">Shopping Cart</h1>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <AnimatePresence mode='popLayout'> 
            {cartItems.map((item) => (
              <motion.div 
                layout // ⭐ 關鍵：讓刪除後的列表自動滑動補位
                key={item.id}
                variants={itemVariants}
                initial="hidden"
                animate="visible"
                exit={{ opacity: 0, x: -100, transition: { duration: 0.2 } }} // 刪除時往左滑出
                className="flex items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 overflow-hidden"
              >
                <img 
                  src={item.image || item.img} 
                  alt={item.name} 
                  className="w-24 h-24 rounded-xl object-cover bg-gray-50 flex-shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <h3 className="text-xl font-bold text-gray-800 truncate">{item.name}</h3>
                  <p className="text-gray-500 text-sm mb-2 truncate">
                    {item.description ? item.description : "Delicious food"}
                  </p>
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-yellow-500 text-lg">${item.price}</span>
                    <span className="bg-gray-100 px-3 py-1 rounded-lg text-sm text-gray-600 font-bold">
                        Qty: {item.quantity}
                    </span>
                  </div>
                </div>
                <motion.button 
                  ...
                  className="p-3 text-gray-400 rounded-full transition-all"
                >
        ...
        <motion.div variants={itemVariants} className="lg:col-span-1">
          <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 sticky top-24">
            <h2 className="text-2xl font-bold font-league mb-6 border-b pb-4">Order Summary</h2>
